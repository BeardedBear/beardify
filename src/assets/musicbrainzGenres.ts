/*
 * The official MusicBrainz genre vocabulary, the exact terms `tag:"…"` matches in the
 * releases query. Shipped as data rather than fetched: the endpoint pages 100 at a
 * time, so reading it live would be 22 requests at the one-second spacing the shared
 * MusicBrainz queue enforces — 24 seconds of that queue, blocking the artist page
 * behind it, to populate one autocomplete.
 *
 * It only feeds suggestions; the field still accepts anything typed, so a genre added
 * upstream since this snapshot is inconvenient, not unreachable.
 *
 * Regenerate with: curl "https://musicbrainz.org/ws/2/genre/all?fmt=json&limit=100&offset=N"
 * (send a real User-Agent, walk `offset` to `genre-count`, keep `name`).
 * Snapshot: 2026-08-28, 2192 genres.
 */
export const MUSICBRAINZ_GENRES: string[] = [
  "2 tone",
  "2-step",
  "3-step",
  "aak",
